...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('modalidad/registrar')}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function filtroModalidad()
    {
        ...
        $.ajax({
          type: "POST",
          url: "{{url('modalidad/listar-modalidad')}}",
          data: {search:search},
          success: function( response ) {
            ...
            var nro_modalidad = $("#tabla").DataTable().rows().count();
            if(nro_modalidad>1){
                $("#titulo").text(nro_modalidad+" Modalidades ");
            }else{
                $("#titulo").text(nro_modalidad+" Modalidad");
            }
    ...
    eliminarModalidad = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('modalidad/eliminar-modalidad')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroModalidad();
