...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('nuestro-numero/registrar')}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function filtroNuestroNumero()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('nuestro-numero/listar-nuestro-numero')}}",
          data: {search:search},
          success: function( response ) {
            ...
            var nro_nuestro_numero = $("#tabla").DataTable().rows().count();
            if(nro_nuestro_numero>1){
                $("#titulo").text(nro_nuestro_numero+" Nuestros números");
            }else{
                $("#titulo").text(nro_nuestro_numero+" Nuestro número");
            }
    ...
    eliminarNuestroNumero = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('nuestro-numero/eliminar-nuestro-numero')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroNuestroNumero();
