...
@section('titulo')
<title>Categorías de cursos</title>
@endsection
...
@section('header')
<h1 class="text-white lh-base fw-bolder roboto-medium fs-1">Categorías de cursos</h1>
@endsection
...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        <div class="col-12 col-lg-6 d-flex align-items-center mb-lg-0">
            <a href="{{url('categoria-curso')}}" class="d-flex align-items-center text-decoration-none">
    ...
    <div class="mb-4">
        <h4 class="fs-2">Modificar categoría</h4>
    </div>
    <form id="formulario">
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                ...
                <input type="text" name="nombre" value="{{$categoria->nombre}}" class="form-control roboto-medium" placeholder="Ingresar nombre" required>
            </div>
            <div class="col-md-6">
                <label class="form-label fw-bolder roboto-medium">Mostrar en inicio <small class="required"></small></label>
                <select name="mostrar_inicio" class="form-select roboto-medium" required>
                    <option value="">Seleccione una opción</option>
                    @foreach($estados as $item)
                        @if($categoria->mostrar_inicio==$item->id)
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                ...
                <input type="number" name="orden" value="{{$categoria->orden}}" class="form-control roboto-medium" placeholder="Ingresar orden" >
            </div>
            <div class="col-md-6">
                ...
                <select name="habilitado" class="form-select roboto-medium" required>
                    <option value="">Seleccione una opción</option>
                    @foreach($estados as $item)
                        @if($categoria->habilitado==$item->id)
                        <option value="{{$item->id}}" selected>{{$item->nombre}}</option>
                        @else
                        <option value="{{$item->id}}">{{$item->nombre}}</option>
                        @endif
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <label class="form-label fw-bolder roboto-medium">Imagen </label>
                    ...
                    Para asegurar la calidad de la imagen debe cumplir con el siguiente peso: ser menor a 10MB y una dimensión aproximada: 100x100 px.
                    </div>
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                <a href="{{url('categoria-curso')}}" class="btn btn-border-white">Cancelar</a>
...
$(document).ready(function() {
     ...
     $('#navCategoriaCurso').addClass('actived');
    ...
    $('#actualizar').on('click', function() {
        ...
        $.ajax({
            type: "POST",
            url: "{{url('categoria-curso/actualizar-categoria-curso')}}",
            ...
            success: function( response ) {
                if (response.success) {
                    ...
                    toastr.options.timeOut = 10000;
                    toastr.options.extendedTimeOut = 10000;
                    setTimeout(function(){window.location = "/categoria-curso"} , 2000);   
                }else{
