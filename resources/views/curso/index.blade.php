@extends('layout.admin')

@section('titulo')
<title>Cursos</title>
@endsection

@section('header')
<h1 class="text-white lh-base fw-bolder roboto-medium fs-1">Cursos</h1>
@endsection

@section('content')
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        <div class="col-12 col-lg-6 d-flex align-items-center mb-3 mb-lg-0">
            <h1 class="text-dark fw-bolder roboto-medium fs-2 my-0" id="titulo"></h1>
        </div>
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            <div class="input-group w-100 w-lg-auto">
                <input type="text" class="form-control roboto-medium" placeholder="Buscar" aria-label="Buscar" id="search">
                <span class="input-group-text cursor-pointer" id="buscar">
                    <i class="bi bi-search"></i>
                </span>
            </div>
            <a href="{{ url('curso/registrar') }}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>

    <table id="tabla" class="table table-row-bordered">
        <thead>
            <tr class="fw-bold fs-7 blue-600">
                <th>ID</th>
                <th>Imagen</th>
                <th>Docente</th>
                <th>Subcategoria</th>
                <th>Nombre</th>
                <th>Precio</th>
                <th>Descuento</th>
                <th>Curso popular</th>
                <th>Curso ideal</th>
                <th>Habilitado</th>
                <th>Pregunta frecuente</th>
                <th>Opinión estudiante</th>
                <th>Acciones</th>
            </tr>
        </thead>
        <tbody id="cuerpo">
        </tbody>
    </table>
</div>

@endsection

@push('scripts')
<script type="text/javascript">
$(document).ready(function() {
    $('#cursos').addClass('here show');
    $('#navCurso').addClass('actived');

    filtroCurso();

    $("#buscar").click(function () {
        filtroCurso();
    });

    function filtroCurso() {
        var search = $('#search').val();
        $.ajax({
            type: "POST",
            url: "{{ url('curso/listar-curso') }}",
            data: { search: search },
            success: function(response) {
                if ($.fn.DataTable.isDataTable('#tabla')) {
                    $('#tabla').DataTable().destroy();
                }
                $('#cuerpo').html(response);

                $("#tabla").DataTable({
                    "ordering": true,
                    "order": [],
                    "columnDefs": [{ "orderable": false, "targets": [0] }]
                });

                var nro_cursos = $("#tabla").DataTable().rows().count();
                $("#titulo").text(nro_cursos > 1 ? nro_cursos + " Cursos" : nro_cursos + " Curso");
            },
            error: function(xhr, ajaxOptions, thrownError) {
                console.log(xhr.responseText);
            }
        });
    }

    eliminarCurso = function(id) {
        $.ajax({
            type: "POST",
            url: "{{ url('curso/eliminar-curso') }}",
            data: { id: id },
            success: function(response) {
                if (response.success) {
                    filtroCurso();
                    toastr.success(response.message, 'Satisfactorio!');
                } else {
                    toastr.error(response.message, 'Ocurrió un error!');
                }
            },
            error: function(xhr, ajaxOptions, thrownError) {
                console.log(xhr.responseText);
            }
        });
    }
});
</script>
@endpush
