...
<div class="container">
    ...
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('curso/opinion-estudiante/registrar/'.$curso->id)}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function filtroOpinionEstudiante()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('curso/opinion-estudiante/listar-opinion-estudiante')}}",
          data: {
            search:search,
            id_curso:curso.id
        },
          success: function( response ) {
            ...
            var nro_opinion_estudiante = $("#tabla").DataTable().rows().count();
            if(nro_opinion_estudiante>1){
                $("#titulo").text(nro_opinion_estudiante+" Opiniones de estudiantes de ''"+curso.nombre+"'");
            }else{
                $("#titulo").text(nro_opinion_estudiante+" Opinión de estudiante del '"+curso.nombre+"'");
            }
    ...
    eliminarOpinionEstudiante = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('curso/opinion-estudiante/eliminar-opinion-estudiante')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroOpinionEstudiante();
