...
@section('header')
<h1 class="text-white lh-base fw-bolder roboto-medium fs-1">Suscripcion</h1>
@endsection
...
$(document).ready(function() {
    ...
    $('#navSuscripcion').addClass('actived');
    ...
    function filtroSuscripcion()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('suscripcion/listar-suscripcion')}}",
          data: {search:search},
          success: function( response ) {
            ...
            if(nro_categoria>1){
                $("#titulo").text(nro_categoria+" Suscripciones ");
            }else{
                $("#titulo").text(nro_categoria+" Suscripción");
            }
    ...
    eliminarSuscripcion = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('suscripcion/eliminar-suscripcion')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroSuscripcion();
