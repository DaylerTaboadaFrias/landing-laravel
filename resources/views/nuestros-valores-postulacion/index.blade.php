...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('nuestros-valores-postulacion/registrar')}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    filtroNuestrosValoresPostulacion();
    ...
    function filtroNuestrosValoresPostulacion()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('nuestros-valores-postulacion/listar-nuestros-valores-postulacion')}}",
          data: {search:search},
          success: function( response ) {
            ...
            var nro_tipo_curso = $("#tabla").DataTable().rows().count();
            if(nro_tipo_curso>1){
                $("#titulo").text(nro_tipo_curso+" Nuestros valores de postulación ");
            }else{
                $("#titulo").text(nro_tipo_curso+" Nuestros valores de postulación");
            }
    ...
    eliminarNuestrosValoresPostulacion = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('nuestros-valores-postulacion/eliminar-nuestros-valores-postulacion')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroNuestrosValoresPostulacion();
