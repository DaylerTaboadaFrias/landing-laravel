...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('motivo-contacto/registrar')}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function filtroMotivoContacto()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('motivo-contacto/listar-motivo-contacto')}}",
          data: {search:search},
          success: function( response ) {
            ...
            if(nro_categoria>1){
                $("#titulo").text(nro_categoria+" Motivos de contacto");
            }else{
                $("#titulo").text(nro_categoria+" Motivo de contacto");
            }
    ...
    eliminarMotivoContacto = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('motivo-contacto/eliminar-motivo-contacto')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroMotivoContacto();
