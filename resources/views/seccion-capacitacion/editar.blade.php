...
@section('titulo')
<title>Sección de capacitación</title>
@endsection
...
@section('header')
<h1 class="text-white lh-base fw-bolder roboto-medium fs-1">Sección de capacitación</h1>
@endsection
...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        <div class="col-12 col-lg-6 d-flex align-items-center mb-lg-0">
            <a href="{{url('seccion-capacitacion')}}" class="d-flex align-items-center text-decoration-none">
    ...
    <div class="mb-4">
        <h4 class="fs-2">Modificar sección de capacitación</h4>
    </div>
    <form id="formulario">
        <input type="hidden" name="id" value="{{$seccionCapacitacion->id}}">
        <div class="row mt-3">
            <div class="col-md-6">
                <label class="form-label fw-bolder roboto-medium">Título <small class="required"></small></label>
                <input type="text" name="titulo" value="{{$seccionCapacitacion->titulo}}" class="form-control roboto-medium" placeholder="Ingresar título" required>
            </div>
            <div class="col-md-6">
                <label class="form-label fw-bolder roboto-medium">Subtítulo <small class="required"></small></label>
                <input type="text" name="subtitulo" value="{{$seccionCapacitacion->subtitulo}}" class="form-control roboto-medium" placeholder="Ingresar subtítulo" required>
            </div>
        </div>
        
        <div class="row mt-3">
            <div class="col-md-12">
                <label class="form-label fw-bolder roboto-medium">Descripción <small class="required"></small></label>
                <textarea name="descripcion" class="form-control roboto-medium" cols="3" rows="2" placeholder="Ingresar descripción"  required>{{$seccionCapacitacion->descripcion}}</textarea>
            </div>
        </div>
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                <label class="form-label fw-bolder roboto-medium">Imagen </label>
                    ...
                    Para asegurar la calidad de la imagen debe cumplir con el siguiente peso: ser menor a 10MB y una dimensión aproximada: 607x538 px.
                    </div>
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                <a href="{{url('seccion-capacitacion')}}" class="btn btn-border-white">Cancelar</a>
...
$(document).ready(function() {
     ...
     $('#navSeccionCapacitacion').addClass('actived');
    ...
    var seccionCapacitacion = {!! json_encode($seccionCapacitacion) !!};
   

    var imagen = seccionCapacitacion.imagen;
    if (imagen!=null) {
    ...
    $('#actualizar').on('click', function() {
        ...
        $.ajax({
            type: "POST",
            url: "{{url('seccion-capacitacion/actualizar-seccion-capacitacion')}}",
            ...
            success: function( response ) {
                if (response.success) {
                    ...
                    toastr.options.timeOut = 10000;
                    toastr.options.extendedTimeOut = 10000;
                    setTimeout(function(){window.location = "/seccion-capacitacion"} , 2000);   
                }else{
