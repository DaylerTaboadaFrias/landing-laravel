...
<div class="container">
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-center flex-column flex-lg-row gap-3">
            ...
            <a href="{{url('docente/registrar')}}" class="btn btn-primary w-100 w-lg-auto d-flex align-items-center">
                Crear Nuevo
            </a>
        </div>
    </div>
    ...
    <table id="tabla" class="table table-row-bordered">
        <thead>
            <tr class="fw-bold fs-7 blue-600">
                <th>ID</th>
                <th>Imagen</th>
                <th>ID del docente externo</th>
                <th>Prefijo academico</th>
                <th>Nombre completo</th>
                <th>Profesión</th>
                <th>Biografía</th>
                <th>Enlace linkedin</th>
                <th>Habilitado</th>
                <th>Acciones</th>
            </tr>
        </thead>
...
$(document).ready(function() {
    ...
    function filtroDocente()
    {
        var search = $('#search').val();
        $.ajax({
          type: "POST",
          url: "{{url('docente/listar-docente')}}",
          data: {search:search},
          success: function( response ) {
            ...
            if(nro_tipo_curso>1){
                $("#titulo").text(nro_tipo_curso+" Docentes ");
            }else{
                $("#titulo").text(nro_tipo_curso+" Docente");
            }
    ...
    eliminarDocente = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('docente/eliminar-docente')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroDocente();
