...
@section('titulo')
<title>Solicitudes de postulación</title>
@endsection
...
@section('header')
<h1 class="text-white lh-base fw-bolder roboto-medium fs-1">Solicitudes de postulación</h1>
@endsection
...
<div class="container">
    ...
    <div class="row align-items-center mt-5 mb-5">
        ...
        <div class="col-md-3 mb-3 mb-lg-0">
            <label class="form-label fw-bolder">Disponibilidad <small class="required"></small></label>
            <select id="id_disponibilidad" class="form-select roboto-medium" required>
...
$(document).ready(function() {
    ...
    $('#fecha_inicio, #fecha_fin, #id_disponibilidad, #recibido').on('change', function(){
          filtroSolicitudPostulacion();
    });
    ...
    function filtroSolicitudPostulacion()
    {
        ...
        $.ajax({
          type: "POST",
          url: "{{url('solicitud-postulacion/listar-solicitud-postulacion')}}",
          data: {search:search, fecha_inicio:fecha_inicio , fecha_fin:fecha_fin, recibido:recibido, id_disponibilidad:id_disponibilidad },
          success: function( response ) {
            ...
            var nro_solicitud_postulacion = $("#tabla").DataTable().rows().count();
            if(nro_solicitud_postulacion>1){
                $("#titulo").text(nro_solicitud_postulacion+" Solicitudes de postulación");
            }else{
                $("#titulo").text(nro_solicitud_postulacion+" Solicitud de postulación");
            }
    ...
    eliminarSolicitudPostulacion = function(id){
        $.ajax({
            type: "POST",
            url: "{{url('solicitud-postulacion/eliminar-solicitud-postulacion')}}",
            data: {id: id},
            success: function( response ) {
                if (response.success) {
                    filtroSolicitudPostulacion();
